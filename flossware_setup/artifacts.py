...
def generate_artifacts(config: Config, *, write_agent_files: bool = True) -> dict:
    ...
    (central / STATE_JSON_NAME).write_text(state_json, encoding="utf-8")
    active_providers = (
        "ACTIVE_PROVIDERS = {k: bool(__import__('os').environ.get(v)) "
        "for k, v in PROVIDER_ENV_VARS.items()}\n"
    )
    (central / AI_CONFIG_NAME).write_text(
        (
            "# Auto-generated by FlossWare/agent-setup. No credential values are stored.\n"
    ...
    (central / PATH_TXT_NAME).write_text(str(repo) + "\n", encoding="utf-8")
