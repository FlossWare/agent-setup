...
def run_demo() -> int:
    ...
    schema = ConfigSchema().add(ConfigField("optimization.population", "integer", 30, 1, 500)).add(ConfigField("provider", "string", "anthropic", values=("anthropic",)))
    ...
    order = resolve_order(["models", "validation", "optimization", "agents"], [{"item": "optimization", "after": ["models"], "before": ["validation"]}])
    print("\nORDERING")
    print(" -> ".join(order))
    candidates = [
    ...
    def fitness(x):
        return float(x["quality"]) - 0.12 * float(x["cost"])
    ...
    arms = [Arm("haiku+hybrid", 0.86, 0.25), Arm("sonnet+hybrid", 0.96, 0.80), Arm("sonnet+consensus", 0.98, 1.10)]
    bandit = ThompsonBandit(arms, seed=42)
    for _ in range(30):
        arm = bandit.select()
        bandit.observe(arm, arm.success >= 0.9)
    print("\nTHOMPSON SAMPLING")
    ...
    print(f"\nSELECTED: {selected}")
    print("Policy: PASS | Budget: $300/month hard ceiling | Network: not used")
    print("Explainability: PASS | Reproducibility: PASS")
    return 0
