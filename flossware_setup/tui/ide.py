...
def _shadow(win, top: int, left: int, height: int, width: int) -> None:
    """Paint an offset DOS/Turbo-style shadow without clearing the desktop."""
    h, w = win.getmaxyx()
    sx, sy = left + 2, top + 1
    for y in range(sy, min(h, top + height + 1)):
        try:
            win.addstr(y, sx, " " * max(0, min(width, w - sx - 1)), curses.A_DIM)
        except curses.error:
            pass


def _popup(win, top: int, left: int, height: int, width: int, title: str):
    _shadow(win, top, left, height, width)
    popup = curses.newwin(height, width, top, left)
    popup.keypad(True)
    popup.bkgd(" ")
    try:
        popup.box()
        popup.addstr(0, 2, f" {title} ", curses.A_BOLD)
    except curses.error:
        pass
    return popup
...
def _popup_menu(win, name: str, x: int) -> str | None:
    ...
    h, w = win.getmaxyx()
    width = min(max((len(i) for i in items), default=10) + 6, max(14, w - x - 2))
    height = min(len(items) + 2, max(3, h - 3))
    popup = _popup(win, 1, x, height, width, name)
    cursor = 0
    while True:
        for i, item in enumerate(items[: height - 2]):
            try:
                popup.addstr(1 + i, 1, " " + item.ljust(width - 3)[: width - 3],
                             curses.A_REVERSE if i == cursor else 0)
            except curses.error:
                pass
        popup.noutrefresh(); curses.doupdate()
        key = popup.getch()
        if key == curses.KEY_MOUSE:
            ...
            if event:
                ...
                if bstate & (getattr(curses, "BUTTON1_CLICKED", 0) | getattr(curses, "BUTTON1_PRESSED", 0)):
                    if x <= mx < x + width and 2 <= my < 2 + len(items):
                        return items[my - 2]
                    return None
...
def profile_selector(win) -> str | None:
    ...
    h, w = win.getmaxyx()
    height = min(len(profiles) + 4, max(7, h - 4))
    width = min(max(34, max(map(len, profiles)) + 10), max(20, w - 6))
    top, left = max(2, (h - height) // 2), max(2, (w - width) // 2)
    popup = _popup(win, top, left, height, width, "Select Profile")
    while True:
        for i, profile in enumerate(profiles[: height - 4]):
            label = profile.replace("-", " ").title()
            try:
                popup.addstr(2 + i, 2, ("> " if i == cursor else "  ") + label,
                             curses.A_REVERSE if i == cursor else 0)
            except curses.error:
                pass
        try:
            popup.addstr(height - 2, 2, "Enter Select   Esc Cancel   Mouse", curses.A_DIM)
        except curses.error:
            pass
        popup.noutrefresh(); curses.doupdate()
        key = popup.getch()
        if key == curses.KEY_MOUSE:
            ...
            if event:
                ...
                if bstate & (getattr(curses, "BUTTON1_CLICKED", 0) | getattr(curses, "BUTTON1_PRESSED", 0)):
                    local = y - top - 2
                    if 0 <= local < min(len(profiles), height - 4):
                        save_active_profile(profiles[local]); return profiles[local]
        elif key in (curses.KEY_UP, ord("k")):
            cursor = max(0, cursor - 1)
        elif key in (curses.KEY_DOWN, ord("j")):
            cursor = min(len(profiles) - 1, cursor + 1)
        elif key in (10, 13, curses.KEY_ENTER):
            save_active_profile(profiles[cursor]); return profiles[cursor]
        elif key == 27:
            return None
...
def _draw_box(win, top: int, left: int, bottom: int, right: int, title: str) -> None:
    try:
        ...
        win.vline(top+1, left, curses.ACS_VLINE, max(0, bottom-top-1)); win.vline(top+1, right, curses.ACS_VLINE, max(0, bottom-top-1))
        win.addch(bottom, left, curses.ACS_LLCORNER); win.hline(bottom, left+1, curses.ACS_HLINE, max(0, right-left-1)); win.addch(bottom, right, curses.ACS_LRCORNER)
        add(win, top, left+2, f" {title} ", 1, curses.A_BOLD)
    except curses.error:
        pass


def _run_menu_action(win, action: str | None) -> str | None:
...
def run(win) -> None:
    palette(); win.keypad(True); enable_mouse(); active = load_active_profile()
    while True:
        win.erase(); h, w = win.getmaxyx(); config = _effective_profile_config(active); _draw_main(win)
        left = max(20, min(29, w // 4)); _draw_box(win, 2, 1, max(3, h-5), left, "Profiles")
        profiles = available_profiles()
        for i, profile in enumerate(profiles):
            if 4+i >= h-5: break
            add(win, 4+i, 3, f"{'>' if profile == active else ' '} {profile.replace('-', ' ').title()}", 2 if profile == active else 5, curses.A_BOLD if profile == active else 0)
        ...
        add(win, h-2, 2, "F-keys optional   |   Esc/Q Exit", 6); win.refresh(); key = win.getch()
        if key == curses.KEY_MOUSE:
            event = mouse_event()
            if event:
                ...
                if bstate & clicked:
                    if y == 0:
                        for menu_name, lx, rx in _menu_x_positions():
                            if lx <= x <= rx:
                                chosen = _run_menu_action(win, _popup_menu(win, menu_name, lx))
