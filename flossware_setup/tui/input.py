...
def primary_button_mask() -> int:
    ...
    return int(getattr(curses, "BUTTON1_CLICKED", 0) | getattr(curses, "BUTTON1_PRESSED", 0))
...
def list_index_at(y: int, origin_y: int, count: int) -> int | None:
    """Map a screen row to a list index, or None when outside the list."""
    if count <= 0:
        return None
    index = int(y) - int(origin_y)
...
def resolve_list_mouse(
    ...
    *,
    ...
    if event is None or count <= 0:
        return None
    _x, y, bstate = event
    index = list_index_at(y, origin_y, count)
    if index is None:
        return None
    if is_primary_click(bstate):
