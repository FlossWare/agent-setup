...
for required in scripts/setup.py scripts/profile.sh scripts/flossware-ai scripts/router_mcp.py scripts/discovery.py scripts/mcp.py scripts/tui.py scripts/setup_tui.py scripts/agent_setup.py scripts/runtime.py; do [[ -f "$SETUP_DIR/$required" ]] || fail "missing $required"; done
python -m compileall -q "$SETUP_DIR/scripts/setup.py" "$SETUP_DIR/scripts/tui.py" "$SETUP_DIR/scripts/setup_tui.py" "$SETUP_DIR/scripts/agent_setup.py" "$SETUP_DIR/scripts/router_mcp.py" "$SETUP_DIR/scripts/discovery.py" "$SETUP_DIR/scripts/mcp.py" "$SETUP_DIR/scripts/runtime.py"
...
EOF
chmod 700 "$PATH_SHIM"
log "Installation complete: $INSTALL_ROOT"; printf '%s\n' "Profile: $PROFILE" "Platform: $PLATFORM" "Run: flossware-ai tui" "Run: flossware-ai doctor"
