...
install_prereqs(){
  case "$PLATFORM" in
    fedora) if [[ $EUID -eq 0 ]]; then PM=(dnf); else command -v sudo >/dev/null 2>&1 || fail "sudo is required."; PM=(sudo dnf); fi; log "Installing Fedora/RHEL-family prerequisites"; "${PM[@]}" install -y git python3 python3-devel python3-pip gcc gcc-c++ make pkgconf-pkg-config openssl-devel libffi-devel rust cargo ncurses-devel;;
    debian) if [[ $EUID -eq 0 ]]; then PM=(apt-get); else command -v sudo >/dev/null 2>&1 || fail "sudo is required."; PM=(sudo apt-get); fi; log "Installing Debian-family prerequisites"; "${PM[@]}" update; "${PM[@]}" install -y git python3 python3-venv python3-dev python3-pip build-essential pkg-config libssl-dev libffi-dev rustc cargo libncurses-dev;;
    freebsd) if [[ $EUID -eq 0 ]]; then PM=(pkg); else command -v doas >/dev/null 2>&1 && PM=(doas pkg) || { command -v sudo >/dev/null 2>&1 || fail "sudo or doas is required."; PM=(sudo pkg); }; fi; log "Installing FreeBSD prerequisites"; "${PM[@]}" install -y git python3 py311-pip gcc pkgconf openssl libffi rust;;
    termux) command -v pkg >/dev/null 2>&1 || fail "Termux pkg is required."; log "Installing Termux prerequisites"; pkg update -y; pkg install -y git python python-pip clang make pkg-config openssl libffi rust;;
  esac
}
install_prereqs
python3 - <<'PY'
...
if [[ "$USE_SOURCE" == true ]]; then log "Source mode enabled: installing coding-agent-ai from $RELEASE_REF"; python -m pip install --upgrade "coding-agent-ai[all,tui] @ git+$AI_REPO@$RELEASE_REF"; else log "Installing coding-agent-ai from published artifacts when available"; if ! python -m pip install --upgrade --prefer-binary "coding-agent-ai[all,tui]"; then log "Published artifact unavailable or incomplete; falling back to source $RELEASE_REF"; python -m pip install --upgrade "coding-agent-ai[all,tui] @ git+$AI_REPO@$RELEASE_REF"; fi; fi
...
for required in scripts/setup.py scripts/profile.sh scripts/flossware-ai scripts/router_mcp.py scripts/discovery.py scripts/mcp.py scripts/tui.py scripts/agent_setup.py scripts/runtime.py; do [[ -f "$SETUP_DIR/$required" ]] || fail "missing $required"; done
python -m compileall -q "$SETUP_DIR/scripts/setup.py" "$SETUP_DIR/scripts/tui.py" "$SETUP_DIR/scripts/agent_setup.py" "$SETUP_DIR/scripts/router_mcp.py" "$SETUP_DIR/scripts/discovery.py" "$SETUP_DIR/scripts/mcp.py" "$SETUP_DIR/scripts/runtime.py"
...
for f in profile.sh profile.toml; do :; done
...
PY
log "Installation complete"; printf '%s\n' "AI root: $INSTALL_ROOT" "Profile: $PROFILE" "Launcher: $PATH_SHIM" "Platform: $PLATFORM" "Ref: $RELEASE_REF" "" "Run: flossware-ai tui" "Run: flossware-ai doctor" "Run: flossware-ai runtime status" "Run: flossware-ai components" "Run: flossware-ai accounts --verify" "Run: flossware-ai models --refresh" "Reinstall: ./scripts/install.sh --reinstall" "Clean: ./scripts/install.sh --clean"
