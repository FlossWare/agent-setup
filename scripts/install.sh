...
REINSTALL=false
CLEAN=false
INSTALL_ROOT="${FLOSSWARE_AI_HOME:-${FLOSSWARE_INSTALL_ROOT:-$HOME/.FlossWare/ai}}"
VENV="$INSTALL_ROOT/venv"
SETUP_DIR="$INSTALL_ROOT/agent-setup"
AI_REPO="https://github.com/FlossWare/agent-ai.git"
SETUP_REPO="https://github.com/FlossWare/agent-setup.git"
RELEASE_REF="${FLOSSWARE_RELEASE_REF:-main}"
AI_REF="${FLOSSWARE_AI_REF:-main}"
USE_SOURCE="${FLOSSWARE_USE_SOURCE:-false}"

if [[ "$RELEASE_REF" =~ ^[0-9a-f]{40}$ ]]; then
  SETUP_ARCHIVE="https://codeload.github.com/FlossWare/agent-setup/tar.gz/$RELEASE_REF"
else
  SETUP_ARCHIVE="https://codeload.github.com/FlossWare/agent-setup/tar.gz/refs/heads/$RELEASE_REF"
fi
...
case "$AGENT" in
  claude|claude-code|cursor|opencode|crush|codex|aider|cline|roo-code|gemini-cli|github-copilot|windsurf|amazon-q|kiro|all) ;;
...
if [[ -n "${TERMUX_VERSION:-}" || "${PREFIX:-}" == /data/data/com.termux/* ]]; then
  ...
  case "${ID:-}" in
    fedora|rhel|rocky|almalinux|ol|centos|nobara) PLATFORM="fedora";;
...
install_prereqs() {
  ...
  case "$PLATFORM" in
    fedora)
      PM=(dnf); [[ $EUID -eq 0 ]] || PM=(sudo dnf)
      "${PM[@]}" install -y curl tar git python3 python3-devel python3-pip gcc gcc-c++ make pkgconf-pkg-config openssl-devel libffi-devel rust cargo ncurses-devel
      ;;
    debian)
      PM=(apt-get); [[ $EUID -eq 0 ]] || PM=(sudo apt-get)
      "${PM[@]}" update
      "${PM[@]}" install -y curl tar git python3 python3-venv python3-dev python3-pip build-essential pkg-config libssl-dev libffi-dev rustc cargo libncurses-dev
      ;;
    freebsd)
      PM=(pkg); [[ $EUID -eq 0 ]] || { command -v doas >/dev/null 2>&1 && PM=(doas pkg) || PM=(sudo pkg); }
      "${PM[@]}" install -y curl tar git python3 py311-pip gcc pkgconf openssl libffi rust
      ;;
    termux)
      pkg update -y
      pkg install -y curl tar git python python-pip clang make pkg-config openssl libffi rust
      ;;
  esac
}
...
if [[ "$USE_SOURCE" == true ]]; then
  python -m pip install --upgrade "agent-ai[all,tui] @ git+$AI_REPO@$AI_REF"
else
  python -m pip install --upgrade --prefer-binary "agent-ai[all,tui]" \
    || python -m pip install --upgrade "agent-ai[all,tui] @ git+$AI_REPO@$AI_REF"
fi
...
if [[ "$USE_SOURCE" == true ]]; then
  ...
  rm -rf "$SETUP_DIR"
  curl -fsSL "$SETUP_ARCHIVE" -o "$TMP_SETUP/setup.tar.gz"
  tar -xzf "$TMP_SETUP/setup.tar.gz" -C "$TMP_SETUP"
  EXTRACTED="$(find "$TMP_SETUP" -mindepth 1 -maxdepth 1 -type d \( -name 'agent-setup-*' -o -name 'coding-agent-setup-*' \) -print -quit)"
  [[ -n "$EXTRACTED" ]] || fail "failed to unpack agent-setup artifact"
  mv "$EXTRACTED" "$SETUP_DIR"
fi

for required in scripts/setup.py scripts/profile.sh scripts/write-install-metadata.sh flossware_setup/tui/app.py scripts/flossware-ai scripts/router_mcp.py scripts/discovery.py scripts/mcp.py scripts/tui.py scripts/agent_setup.py scripts/runtime.py scripts/dogfood.py; do
  [[ -f "$SETUP_DIR/$required" ]] || fail "missing $required"
done
...
mkdir -p "$PROFILE_DIR" "$INSTALL_ROOT/bin" "$INSTALL_ROOT/state" "$INSTALL_ROOT/cache" "$INSTALL_ROOT/mcp"
...
chmod 700 "$PROFILE_DIR/profile.sh" "$INSTALL_ROOT/bin/flossware-ai" "$INSTALL_ROOT"/*.py
printf '%s\n' "$PROFILE" > "$INSTALL_ROOT/state/active-profile"
chmod 600 "$INSTALL_ROOT/state/active-profile"
...
log "Installation complete: $INSTALL_ROOT"
