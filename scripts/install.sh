...
if [[ "$USE_SOURCE" == true ]]; then
  ...
  TMP_SETUP="$(mktemp -d)"; trap 'rm -rf "$TMP_SETUP"' EXIT; rm -rf "$SETUP_DIR"; curl -fsSL "$SETUP_ARCHIVE" -o "$TMP_SETUP/setup.tar.gz"; tar -xzf "$TMP_SETUP/setup.tar.gz" -C "$TMP_SETUP"; EXTRACTED="$(find "$TMP_SETUP" -mindepth 1 -maxdepth 1 -type d -name 'coding-agent-setup-*' -print -quit)"; [[ -n "$EXTRACTED" ]] || fail "failed to unpack coding-agent-setup artifact"; mv "$EXTRACTED" "$SETUP_DIR"
fi
